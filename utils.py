...
def reconstruct(reduced_FoV, S, psi, Lambda, R): 
    ...
    for m in range(Size_red):
        for n in range(Size): 
            ...
            s = S[indices, n, :].transpose()
            A = reduced_FoV[m, n, :]
            reconstructed [indices, n] = np.dot(
                np.dot(np.linalg.pinv(np.dot(np.dot (A, psi_1), A.transpose()) + Lambda*np.eye (Nc)), np.dot (A, psi_1)), s) 
    return reconstructed
...
# Question 4
def reconstruct_tikhonov(reduced_FoV, S, lambda_val):
    [Size_red, Size, Nc] = reduced_FoV.shape
    delta = round(Size_red/2)
    reconstructed = np.zeros((Size, Size))
    psi = np.identity(Nc)
    ...
    for m in range(Size_red):
        for n in range(Size):
            ...
            # Calcul de la solution de Tikhonov
            A_pinv = np.linalg.pinv(np.dot(A, psi))
            x = np.dot(A_pinv, s)
            reconstructed[indices, n] = x
